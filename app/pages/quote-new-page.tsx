...
export function QuoteNewPage() {
  ...
  const previewFetchRef = useRef<AbortController | null>(null)
  const saveFetchRef = useRef<AbortController | null>(null)
  ...
  useEffect(
    () => () => {
      if (draftTimer.current) clearTimeout(draftTimer.current)
      previewFetchRef.current?.abort()
      saveFetchRef.current?.abort()
    },
    [],
  )
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!res.ok) {
          if (!ac.signal.aborted) applyFallbackTypes()
          return
        }
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const res = await fetch(apiUrl('/api/depot'), { signal: ac.signal })
        if (!res.ok) return
  ...
  const runCalculate = async () => {
    ...
    try {
      ...
      if (previewFetchRef.current === ac) previewFetchRef.current = null
      if (!ac.signal.aborted) setCalcLoading(false)
    }
  }
  ...
  const runSave = async () => {
    ...
    try {
      ...
      router.push(`/quotes/${quoteDetailPathSegment(saved)}`)
    } catch (e) {
      if (ac.signal.aborted) return
      setError(e instanceof Error ? e.message : 'Ошибка сохранения')
    } finally {
      if (saveFetchRef.current === ac) saveFetchRef.current = null
      if (!ac.signal.aborted) setSaveLoading(false)
    }
  }
  ...
  return (
    <div className="mx-auto w-full max-w-[1600px] px-6 py-6">
        ...
        <CardContent className="space-y-4">
          {vehicleTypesDemo && (
            <p className="bg-amber-50 text-amber-950 rounded-md border border-amber-200/80 px-3 py-2 text-sm">
              ...
              Чтобы считать по-настоящему, заполните справочники в вашей БД и
              проверьте `DATABASE_URL`/`/api/vehicle-types`.
            </p>
          )}
      ...
      <Card>
        ...
        <aside className="flex min-h-[280px] min-w-0 w-full flex-col self-stretch xl:sticky xl:top-6 xl:min-h-0 xl:w-(--quote-aside-px) xl:max-w-[min(68vw,1200px)] xl:min-w-[260px] xl:shrink-0">
